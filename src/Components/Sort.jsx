...
function Sort() {
  ...
  let selectActive = (obj) => {
    dispatch(setFilter(obj));
    setOpen(false);
  };

  React.useEffect(() => {
    const clickSortRef = (event) => {
      ...
      document.body.addEventListener('click', clickSortRef);

      return () => document.body.removeEventListener('click', clickSortRef);
    }, [])
    ...
    <div ref={sortRef} className="sort">
      ...
      {open && (
        <div className="sort__popup">
          <ul>
            {selects.map((obj, i) => (
              <li key={i} onClick={() => selectActive(obj)} className={filter.sortProperty === obj.sortProperty ? 'active' : ''}>
                {obj.name}
              </li>
            ))}
          </ul>
